...
def index(request):
	if request.method == "POST":
		...
		if Paciente.objects.filter(id_name=int(request.POST['id_name'])).exists():
			...
			principal_dir=settings.MEDIA_ROOT+'NoWER'+'/'
			date = str(datetime.today().strftime('%Y-%m-%d'))
			if not os.path.isdir(principal_dir+obj.id_name+'/'+date):
				os.makedirs(principal_dir+obj.id_name+'/'+date)
			if not os.path.isdir(settings.MEDIA_ROOT+obj.id_name+'/AUDIOS'):
				os.makedirs(settings.MEDIA_ROOT+obj.id_name+'/AUDIOS')
		...
		if Paciente.objects.filter(id_name=int(request.POST['id_name'])).exists():
			...
			for key in request.POST.keys():
						if key != 'id_name' and key != 'number_session':
							...
							f.write(Move64)
							f.close()
			return HttpResponse('Archivos de movimiento guardados exitosamente')
		else:
			return HttpResponse('El paciente no ha sido creado en el servidor')
